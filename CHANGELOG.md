...
- Tier B omega-lock citation guard for the cross-repo `src/omega_lock/<file>.py:line` references in the READMEs:
  - `scripts/check_omega_lock_citations.py` verifies each omega-lock citation against a *pinned* omega-lock source checkout, reusing antemortem's own `verify_citation` (file-exists + line-in-range) plus a per-file semantic token check that catches a cited construct moving off its line while staying in range.
  - A dedicated, isolated CI job that checks out omega-lock at an immutable pinned SHA (`actions/checkout` with a fixed ref) and runs the guard, so a checkout/network hiccup reds only that signal and never the core trust-checks gate.
  - A namespace-invariant test asserting the guard's contract and that this stays a CI-time check only: a CI-time checkout is not a runtime dependency, and `src/` keeps zero `import omega_lock`.
- Added `.github/workflows/publish.yml` for trusted PyPI publishing (OIDC `id-token: write`, no stored password) on GitHub release publication and manual dispatch.

### Changed

- Re-pinned `README.md` repository links from the `v0.10.4` tag to `v0.10.5`.
- Bumped the package version, PyPI badge, and current-release callout to v0.10.5 across `README.md` and `README_KR.md`.
...
- Added v0.10.3 release notes with absolute, tag-pinned repository links.

### Changed

- Linked the trust model, toolkit positioning, and CLI example gallery docs back to the README.
- Re-pinned `README.md` repository links from the `v0.10.2` tag to `v0.10.3`.
