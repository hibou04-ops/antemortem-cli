...
> **현재 릴리스: v0.10.0** — 공개 README claim은 `python scripts/check_repo_consistency.py`로 source of truth와 대조합니다.
...
v0.10.0 는 **alpha**. CLI 계약 (seven commands, flags, exit codes) 은 stable. JSON artifact schema 는 alpha line 안에서 additive 를 우선합니다. Breaking output-shape 변경은 명시적 contract-lock release 로 미룹니다. Prompt iteration 은 offline test, 기록된 artifact, 또는 문서화된 replay command 로 검증 가능한 경우에만 진행합니다.
