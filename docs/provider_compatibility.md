...
| Ollama | `--provider ollama` | `llama3.1` | `none (local; no API key)` | `Ollama /api/chat with format=<JSON Schema>` | Returned JSON is parsed and validated with the same Pydantic artifact schema. SDK exceptions, daemon-down errors, non-JSON output, and schema errors surface as ProviderError. | Requires the ollama package and a running local daemon; small local models hallucinate citations more, so lint is doubly important. |
