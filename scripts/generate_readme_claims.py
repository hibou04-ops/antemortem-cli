...
def extract_naming_from_pyproject(pyproject: dict[str, Any]) -> NamingFacts:
    """Extract repository, distribution, import, and CLI names separately."""
...
- Repository: `{facts.naming.repository_slug}`
- PyPI distribution: `{facts.naming.distribution_name}` `{facts.package_version}`
- Install: `pip install {facts.naming.distribution_name}`
- Python import package: `{facts.naming.import_package}`
- CLI command: `{facts.naming.cli_command}`
- CLI subcommands: {_join_code(facts.cli_commands)}
...
- Benchmark metrics source: machine-readable output from `{BENCHMARK_COMMAND}`; use the command output as the source of truth for current local metrics.
...
- Benchmark command: `{BENCHMARK_COMMAND}`
- Benchmark metrics source: `{BENCHMARK_COMMAND}`의 machine-readable output입니다. 현재 local metrics는 command output을 source of truth로 사용합니다.
- Repository self-check: `{SELF_CHECK_COMMAND}`
- Evidence-bound citations: {facts.evidence_bound_status}
"""
...
def check_claims(root: Path, facts: ClaimFacts) -> list[str]:
    ...
    for rel_path, expected in expected_outputs(facts).items():
        ...
        if actual != expected:
            diff = _short_unified_diff(actual, expected, str(rel_path))
            issues.append(
                f"{rel_path}: generated claims are stale; run --write\n{diff}"
            )
    return issues
