...
- `classifications`: array of objects, one per user-supplied trap. Fields: `id`, `label`, `citation`, `note`. Optional: `evidence_snippet` when a short exact quote from the cited range sharpens verification.
- `new_traps`: array of objects, for risks surfaced by reading the code. Fields: `id` (format `t_new_N`), `hypothesis`, `label` (always `"NEW"`), `citation`, `note`. Optional: `evidence_snippet`.
...
Do not populate `evidence_hash`. The caller computes evidence hashes locally after disk citation verification.
